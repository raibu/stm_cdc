...
//--------------------------------------------------------------
// kompletten Empfangspuffer auslesen
// Ret_wert :
//   0 = nichts vorhanden
//  >0 = Anzahl der empfangenen Zeichen
//--------------------------------------------------------------
uint16_t UB_VCP_DataRx(char *ptr)
{
  uint16_t akt_pos=0;
  uint8_t wert;
  uint32_t temptail;

  // test ob etwas empfangen wurde
  if(APP_tx_in_cnt==0) return(0);
  ...
  do {
    ...
    *(ptr+akt_pos)=wert;
    akt_pos++;
    APP_tx_in_cnt--;
  }while((APP_tx_ptr_head!=APP_tx_ptr_tail) && (APP_tx_in_cnt>0));
